...
def planning(message):
    planning_text = "Я подобрал для тебя несколько советов, которые помогут разнообразить здоровый рацион.\n\n" \
                    ...
                    "📌Рыба, морепродукты — 2 раза в неделю (можно чаще)\n" \
                    "📌Обязательно включай в рацион субпродукты (печень, сердечки)\n" \
                    "📌Каждый день включай разные злаки и бобовые (если разрешены в лечебном протоколе питания)\n" \
                    "📌Каждый день новый овощ, зелень (например, различные виды капусты и т.д.)"
